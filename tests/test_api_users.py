...
def test_register_user_success(client, db_session):
    ...
    resp = client.post(
        "/api/user/register", json={"email": email, "password": password}
    )
...
def test_register_user_already_exists(client, db_session):
    ...
    resp = client.post(
        "/api/user/register", json={"email": email, "password": "password123"}
    )
...
def test_login_user_not_found(client):
    resp = client.post(
        "/api/user/login", json={"email": "nonexistent@example.com", "password": "any"}
    )
...
def test_login_user_wrong_password(client, db_session):
    ...
    client.post(
        "/api/user/register", json={"email": email, "password": "correct-password"}
    )

    resp = client.post(
        "/api/user/login", json={"email": email, "password": "wrong-password"}
    )
...
def test_refresh_token_new_access_token_works(client, db_session):
    ...
    new_token = client.post(
        "/api/user/refresh", json={"refresh_token": refresh_token}
    ).json()["access_token"]

    resp = client.get(
        "/api/user/profile", headers={"Authorization": f"Bearer {new_token}"}
    )
    assert resp.status_code == 200
